class DomVisual {
    ...
    this.domControlSelector = [
      {
        ...
        event: {
          click: () => {
            // Không gọi removePlayAnimation nữa, để updatePlayButton tự xử lý
            eventBus.emit('play')
          }
        }
      },
  ...
  handleChange () {
    eventBus.on('change', ({ state, duration, currentTime }) => {
      ...
      // Cập nhật icon play/pause theo state
  ...
  initSongInfo ({ title, cover }) {
    ...
    // Set background image cho toàn bộ nền
    const bgElement = this.getContainerDom('#bg');
    if (bgElement) {
      bgElement.style.backgroundImage = `url(${cover})`;
    }
  ...
  handleProgressInteraction(pointer, progressBar) {
    ...
    // Thử tìm audio element trước (dễ seek hơn)
    ...
    // Nếu không có audio element, thử BufferSource
      ...
      const buffer = window.av.source.buffer;
      const duration = buffer.duration;
      newTime = duration * percent;
      
      // BufferSource không thể seek trực tiếp, phải dừng và phát lại
      const wasPlaying = window.av.ac.state === 'running';
      
      // QUAN TRỌNG: Xóa callback onended trước khi stop để không trigger playNext
      if (window.av.source) {
        window.av.source.onended = null;
        try {
      ...
      // Tạo source mới và phát từ vị trí mới
      const newSource = window.av.ac.createBufferSource();
      newSource.buffer = buffer;
      newSource.connect(window.av.analyser);
      newSource.start(0, newTime);
      
      // Gán lại callback onended cho source mới
      newSource.onended = () => {
        window.av.onended && window.av.onended();
      };
      
      // Cập nhật reference
      window.av.source = newSource;
      window.av.startTime = window.av.ac.currentTime - newTime;
      window.av.started = true;
      
      // Nếu đang dừng thì suspend lại
      if (!wasPlaying) {
        window.av.ac.suspend();
      }
  ...
  updatePlayButton(state) {
    const playBtn = this.getControlDom('#playBtn');
    if (!playBtn) return;
    ...
    if (state === 'running') {
      ...
      // Đang dừng -> hiện icon play
      icon.className = 'iconfont icon-play1';
  ...
  setVolume(volume) {
    ...
    // Set volume cho audio context
    if (window.av.gainNode) {
      window.av.gainNode.gain.value = volume;
    }
    
    // Set volume cho audio element fallback
    const audioElement = document.getElementById('audio-fallback');
    if (audioElement) {
  ...
  // Cập nhật icon volume theo mức âm lượng
  updateVolumeIcon(volume) {
    const volumeIconMaterial = document.getElementById('volumeIconMaterial');
    if (!volumeIconMaterial) return;
    
    if (volume === 0) {
      volumeIconMaterial.textContent = 'volume_off'; // Mute
    } else if (volume < 0.5) {
      volumeIconMaterial.textContent = 'volume_down'; // Low volume
    } else {
      volumeIconMaterial.textContent = 'volume_up'; // High volume
    }
  }

  // Khởi tạo volume từ localStorage
  initVolume() {
