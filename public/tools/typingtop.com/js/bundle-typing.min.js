...
$((function() {
    ...
    function n() {
        $(".dropdown-toggle.show").length ? o.addClass("show") : o.removeClass("show")
    }
    $(".dropdown-toggle").each((function() {
        ...
        n()
    })), $(".dropdown-menu").on("click", (function(o) {
        o.stopPropagation()
    })), $(".js-notification-close").on("click", (function(o) {
        $(".nav-notification__wrapper, .nav-notification__menu, #overlay").removeClass("show")
    })), $(".nav-tabs-responsive > li button").click((function() {
        $(".nav-tabs-responsive > li").removeClass("active"), $(this).parent().addClass("active"), $(".nav-tabs-responsive").toggleClass("open")
    }))
})), $((function() {
    $(".sidebar-open").click("click", (function() {
        $("body, html").css("overflow", "hidden"), $("#sidebar").addClass("show")
    })), $(".sidebar-close").on("click", (function(o) {
        $("body, html").css("overflow", "visible"), $("#sidebar").removeClass("show")
    }))
}));
! function(e, t) {
...
$(document).ready(function() {
    activeMenuSidebar()
    setTimeout(load_newest(), 250)
    setTimeout(load_notifications(1), 250)

    $('.btn-delete-notification').click(function(e) {
        $('#notificationModal .modal-body .modal-notification__list').html('')
        $('.nav-notification__list').html('')
        $('#dropdownNotification span').text('0')
        $('.btn-delete-notification').hide()
        mark_as_read()
    })
    //$('#modal-notification').click(function (e) {
    //    e.preventDefault();
    //    load_notifications(1);
    //    $('#notificationModal').modal('show')
    //});

    //$('.btn-show-notification').click(function (e) {
    //    e.preventDefault()
    //    load_notifications(1)
    //    $('#notificationModal').modal('show')
    //});

    $('#dropdownNotification').on('show.bs.dropdown', function(e) {
        load_notifications(1)
    })
    lastActive(600000)
})
...
function lastActive(time) {
    try {
        setTimeout(function() {
            $.post("/ajax/user/last-active", {}, function(re) {
                if (re.success) {
                    lastActive(re.data)
                }
            })
...
function load_newest() {
    ...
    $.get('/typing/get_newest', function(res) {
        let i = 0
        const current_items = $('.box-news .box-news__item').length
        if (current_items < DISPLAY_ITEMS) {
            let need_append = DISPLAY_ITEMS - current_items;
            for (var k = 0; k < need_append; k++) {
                $('.box-news').append(res.data[k]);
                i++;
            }
            //res.data.forEach(function (item) {
            //    if (i < DISPLAY_ITEMS) {
            //        $('.box-news').prepend(item)
            //        i++
            //    }
            //})
        }
        var setVal = setInterval(() => {
            if (i < res.data.length) {
                if ($('.box-news .box-news__item').length >= DISPLAY_ITEMS) {
                    $('.box-news').find('.box-news__item:last').remove()
                }
                $('.box-news').prepend(res.data[i])
                $('.box-news .box-news__item').first().fadeIn(500)
                i++
            } else {
...
//Active menu chọn chức năng (Kiểm tra đánh máy/ kiểm tra đánh máy nâng cao/ cuộc thi đánh máy...)
function activeMenuSidebar() {
    var $curl = location.href
    $(".navigation li").each(function(index, ui) {
        var href = $(ui).children("a").attr("href")
        if ($curl.includes(href, 0)) {
...
///Active menu các nút ở trang profile (Hồ sơ/Cài đặt/Thông báo/Đăng xuất)
function activeMenuProfile(is_menu_edu, regex) {
    if (is_menu_edu == 'True') {
        $('.sidebar-vip-user__options .sidebar-vip-user__options-item').removeClass('is-active')
        $('.sidebar-vip-user__options .sidebar-vip-user__options-item').each(function(index, ui) {
            if ($(ui).attr('href').match(regex)) {
                $(ui).addClass('is-active')
                return
            }
        })
    } else {
        $('.sidebar-user__options .sidebar-user__options-item').removeClass('is-active')
        $('.sidebar-user__options .sidebar-user__options-item').each(function(index, ui) {
            if ($(ui).attr('href').match(regex)) {
                $(ui).addClass('is-active')
                return
            }
...
function load_notifications(autoload) {
    var logged_user = $('#loggedin_user_id').attr("value")
    if (logged_user == undefined || logged_user == '') {
        return
    }
    $.get('/thongbao/get_thong_bao', function(res) {
        if (res.success) {
            if (res.total > 0) {
                $('.btn-delete-notification').show()
            }
            $('#dropdownNotification span').text(res.total)
        }
        if (autoload == 1) {
            window.setTimeout(load_notifications, 10000)
        }
        //$('.nav-notification__list').htnl('');
        $('.nav-notification__list').html(res.contents)
        $('.modal-notification__list').html(res.contents)
    })
    //$.ajax({
    //    url: '/thongbao/get_thong_bao',
    //    type: 'GET',
    //    dataType: 'json',
    //    cache: false,
...
function share_on_facebook(href) {
    FB.ui({
        method: 'share',
        href: href,
    }, function(response) {})
...
///Active menu trang quản trị typing education
function activeMenuEduVip(regex) {
    $(".sidebar-vip-user__list .sidebar-vip-user__item").each(function(index, ui) {
        var href = $(ui).attr('href')
        if (href.match(regex)) {
            $(ui).addClass('is-active')
            return
        } else {
            $(ui).removeClass('is-active')
        }
    })
}

function showOrHide(id, callback) {
    $.post('/show-hide-status', {
        id: id
    }).then((res) => {
        callback(res)
    })
...
function deleteText(id, callback) {
    $.post('/delete-de', {
        id: id
    }).then((res) => {
        callback(res)
...
function fireAlert(title, message, type, timer = 1500) {
    var background = ''
    var color = '#fff'
    switch (type) {
        case 'success':
...
function sendMail(host, data) {
    $.ajax({
        type: 'POST',
        async: false,
        url: host + '/ajax/AddMail.ashx',
        data: data,
        dataType: 'json',
        contentType: false,
        processData: false,
        ...
        success: function(response) {
            if (response.Status) {
                checkStatusSend(host, response.Data)
            } else {
...
function checkStatusSend(host, res) {
    let arr = []
    if (res != null) {
        res.forEach(item => {
            arr.push(item.Id)
        ...
        $.ajax({
            url: host + '/ajax/CheckStatus.ashx',
            type: 'POST',
            data: data,
            dataType: 'json',
            contentType: false,
...
// --- MOCK API FOR LOCAL STATIC OFFLINE VERSION ---
const originalFetch = window.fetch;
window.fetch = async function(url, options) {
    if (typeof url === 'string' && url.includes('/speedtest/get_words')) {
        if (window.location.pathname.includes('practice.html')) {
            const urlParams = new URLSearchParams(window.location.search);
            const id = urlParams.get('id');
            if (id && window.sp) {
                const { data, error } = await window.sp.from('practice_texts').select('*').eq('id', id).single();
                if (data && data.content) {
                    const titleEl = document.getElementById('practice-title');
                    if (titleEl) titleEl.innerText = data.title || 'Văn bản luyện tập';
                    const authorEl = document.getElementById('practice-author');
                    if (authorEl) {
                        const d = new Date(data.created_at);
                        authorEl.innerHTML = `Tạo ngày ${!isNaN(d) ? d.toLocaleDateString('vi-VN') : ''}`;
                    }
                    const lenEl = document.getElementById('practice-length');
                    if (lenEl) lenEl.innerText = data.length || data.content.length;
                    const compEl = document.getElementById('practice-completions');
                    if (compEl) compEl.innerText = data.completions || 0;
                    const rateEl = document.getElementById('practice-rating');
                    if (rateEl) rateEl.innerText = (data.rating || 5).toFixed(1);
                    return {
                        ok: true,
                        json: async () => ({
                            data: { word: data.content, time: 999999 }
                        })
                    };
                }
            }
        ...
        let timeLimit = 60;
        if (mode.includes('15')) timeLimit = 15;
        else if (mode.includes('30')) timeLimit = 30;
        else if (mode.includes('45')) timeLimit = 45;
        else if (mode.includes('3')) timeLimit = 180;
        else if (mode.includes('5')) timeLimit = 300;
        
        let mockWords = "với và của các là được trong đã cho có những người không để từ đó sẽ này cũng tại nhưng khi về việc ra nhiều phải đến theo như hay vào lại sau đang đây có thể chỉ thì rất sự làm còn các bạn hoặc chưa mà tới vì thế đều nên vậy luôn quá thật thế nào nữa đi sao bao đâu đấy nhỉ nhé hả hử nha nghen thế kia thôi xong nhé này nọ lọ chai vâng dạ ạ bẩm thưa chớ đừng nên cần mong muốn hy vọng thích vui buồn đau khổ yêu ghét nhớ quên thương xót hiểu biết nghĩ ngợi nói cười khóc than kêu la hát hò nhảy múa đi đứng nằm ngồi ăn uống ngủ nghỉ thức dậy làm việc học tập chơi đùa xem nghe đọc viết tính toán suy nghĩ quyết định chọn lựa thay đổi giữ gìn phá hủy xây dựng bảo vệ tấn công chạy trốn tìm kiếm trốn tránh gặp gỡ chia tay yêu đương kết hôn sinh con nuôi dạy chăm sóc quan tâm lo lắng giúp đỡ hỗ trợ ủng hộ phản đối đồng ý từ chối chấp nhận tha thứ xin lỗi cảm ơn chào hỏi tạm biệt chúc mừng chia buồn";
        // make it even longer by duplicating and shuffling
        mockWords = mockWords + " " + mockWords + " " + mockWords;
    ...
    if (typeof url === 'string' && (url.includes('/typing/calculate') || url.includes('/typing/luu_kqua'))) {
        let reqBody = {};
        if (options && options.body) {
            try { reqBody = JSON.parse(options.body); } catch(e) {}
        }
        const wpm = (reqBody.keystrokes / 5) / (reqBody.time_complete / 60);
        const accuracy = (reqBody.word_correct / (reqBody.word_correct + reqBody.word_wrong)) * 100;
        return {
            ok: true,
            json: async () => {
                const rVal = `mock&wpm=${Math.floor(wpm) || 0}&acc=${(accuracy || 0).toFixed(1)}&cw=${reqBody.word_correct || 0}&ww=${reqBody.word_wrong || 0}`;
                return {
                    success: true,
                    r: rVal,
                    data: { wpm: Math.floor(wpm) || 0, accuracy: (accuracy || 0).toFixed(1) + " %", r: rVal }
                };
            }
        };
    }
...
const TypingTest = (() => {
    ...
    const state = {
        ...
        isExtendingWords: false,
    };

    // Cache các element để truy cập nhanh hơn
    const elements = {
        inputfield: document.getElementById("inputfield"),
        row1: document.getElementById("row1"),
        reloadBtn: document.getElementById("reload-btn"),
        timer: document.getElementById("timer"),
        ajaxLoad: document.getElementById("ajax-load"),
        reloadBox: document.getElementById("reload-box"),
        words: document.getElementById("words"),
        wordlist: document.getElementById("wordlist"),
        typingMode: document.getElementById("typing-mode"),
        loggedInUserId: document.getElementById("loggedin_user_id"),
        reloadTestTyping: document.getElementById("reload-test-typing")
    };

    // --- LOGIC CỐT LÕI CỦA BÀI TEST ---

    function restart() {
        resetState();
        loadingAnimate();

        const mode = elements.typingMode ?.getAttribute("value") || '60_seconds';
        fetch(`/speedtest/get_words?mode=${mode}`)
    ...
    function resetState() {
        try {
            ...
            elements.row1.style.transform = "translateY(0px)";
            elements.row1.innerHTML = '';
    ...
    function handleWordsData(data) {
        try {
            //if (document.location.href.indexOf('typing-test') === -1) return;
            if (!data ?.data ?.word) {
            ...
            state.max_time = data.data.time;
            state.countdown = data.data.time;
            set_time(state.max_time);

            const word_string = window.location.pathname.includes('practice.html') 
                ? data.data.word.split(/[\s|]+/g).filter(Boolean).join("|") 
                : randomize_text(data.data.word);
            state.words = word_string.split("|");
            ...
            elements.row1.appendChild(fragment);

            state.wordSpans = Array.from(elements.row1.getElementsByTagName('span'));

            const firstWord = state.wordSpans[0];
            if (firstWord) {
                state.line_height = firstWord.offsetHeight || 54;
                firstWord.classList.add("highlight");
            }

            stopLoadingAnimate();
            slideDown(elements.words);
    ...
    function handleInput() {
        if (state.isTestFinished || state.isComposing) return;
        ...
        if (currentInput.endsWith(" ")) {
            const typedWord = currentInput.slice(0, -1);
            if (typedWord) {
                processWordCompletion(typedWord);
            }
            elements.inputfield.value = "";
        } else {
            checkPartialTyping(currentInput);
        }
    }

    function processWordCompletion(typedWord) {
        const currentWordSpan = state.wordSpans[state.word_pointer];
        if (!currentWordSpan) return;

        const expectedWord = state.words[state.word_pointer];
        state.keystrokes_counter += typedWord.length + 1;

        currentWordSpan.classList.remove("highlight", "highlight-wrong");
        currentWordSpan.classList.add(typedWord === expectedWord ? "correct" : "wrong");

        state.user_input_stream += typedWord + " ";
        state.word_pointer++;

        if (!state.isExtendingWords && state.word_pointer >= state.wordSpans.length - 20) {
            if (!window.location.pathname.includes('practice.html')) {
                extendWordList();
            }
        }

        const nextWordSpan = state.wordSpans[state.word_pointer];
        if (nextWordSpan) {
            nextWordSpan.classList.add("highlight");
            checkLineChange(nextWordSpan);
        } else if (window.location.pathname.includes('practice.html')) {
            finishTest();
        }
    }

    async function extendWordList() {
        state.isExtendingWords = true;
        try {
            const mode = elements.typingMode ?.getAttribute("value") || '60_seconds';
            const response = await fetch(`/speedtest/get_words?mode=${mode}`);
            const data = await response.json();

            if (data ?.data ?.word) {
                const newWordsArray = randomize_text(data.data.word).split("|");
                state.words.push(...newWordsArray);
            ...
            console.error("Failed to extend word list:", error);
        } finally {
            state.isExtendingWords = false;
        }
    }

    function checkPartialTyping(currentInput) {
        const currentWordSpan = state.wordSpans[state.word_pointer];
        if (!currentWordSpan) return;

        const expectedWord = state.words[state.word_pointer];
        const isCorrectSoFar = expectedWord.startsWith(currentInput);

        currentWordSpan.classList.toggle("highlight", isCorrectSoFar);
        currentWordSpan.classList.toggle("highlight-wrong", !isCorrectSoFar);
    }

    function checkLineChange(nextWordSpan) {
        const p = nextWordSpan.offsetTop;
        if (p > state.previous_position_top) {
            state.row_counter++;
            state.previous_position_top = p;
            elements.row1.style.transform = `translateY(${-1 * state.line_height * state.row_counter}px)`;
        }
    }
    ...
    function startCountdown() {
        ...
        function timerTick() {
            const elapsedTime = (Date.now() - state.start_time) / 1000;
            const remainingTime = Math.max(0, state.max_time - elapsedTime);

            if (window.location.pathname.includes('practice.html')) {
                set_time(Math.floor(elapsedTime));
                const nextTick = 1000 - (elapsedTime * 1000 % 1000);
                state.timerId = setTimeout(timerTick, nextTick);
            } else {
                set_time(Math.ceil(remainingTime));
    ...
    async function finishTest() {
        ...
        const correctWords = elements.row1.getElementsByClassName('correct').length;
        const wrongWords = elements.row1.getElementsByClassName('wrong').length;
        ...
        slideUp(elements.words);

        const timeComplete = Math.min(state.max_time, Math.round((Date.now() - state.start_time) / 1000));
        const grossWPM = (state.keystrokes_counter / 5) / (timeComplete / 60);
        let net_wpm = Math.max(0, Math.floor(grossWPM - wrongWords / (timeComplete / 60)));
        let accuracy = correctWords + wrongWords > 0 ? (correctWords * 100) / (correctWords + wrongWords) : 0;
        accuracy = Math.round(accuracy * 100) / 100;

        const send_data = {
            typing_mode: elements.typingMode ?.getAttribute("value"),
            ngon_ngu: getCookie('_language_ck'),
            keystrokes: state.keystrokes_counter,
            word_correct: correctWords,
            word_wrong: wrongWords,
            time_complete: timeComplete,
            afk_timer: 0,
            backspace_counter: state.correction_counter,
            user_input: state.user_input_stream,
            wordlist: elements.wordlist ?.textContent || '',
            max_time: state.max_time,
        };

        const logged_user = elements.loggedInUserId ?.getAttribute("value");
        let result_query_param;
        let gamification = null;

        if (!logged_user) {
            try {
                const response = await fetch('/typing/calculate', {
                    method: "POST",
                    headers: {
                ...
                const data = await response.json();
                result_query_param = data.r; // token ký số do server cấp — không tự dựng ở client
            } catch (e) {
                result_query_param = null; // tránh tạo link giả khi lỗi
            }
        } else {
            try {
                // Lấy r param từ mock calculate
                const response = await fetch("/typing/calculate", {
                    method: "POST",
                ...
                result_query_param = res.r;
                net_wpm = res.data.wpm;
                const rawAcc = parseFloat(String(res.data.accuracy).replace(/[^0-9.,]/g, '').replace(',', '.'));
                accuracy = isNaN(rawAcc) ? 0 : rawAcc;
                
                // Lưu kết quả vào Supabase (bảng typing_results)
                if (window.sp) {
                    const mode = document.location.href.includes('advanced-typing-test') ? 'advanced' : 'normal';
                    await window.sp.from('typing_results').insert([
                        {
                            user_id: logged_user,
                            wpm: net_wpm,
                            accuracy: accuracy,
        ...
        const resultUrl = document.location.href.includes('advanced-typing-test') ? 'result-advanced-typing-test.html' : 'result-typing-test.html';
        // Stash gamification (XP/streak/level) + meta for the result page to display once, then clear.
        try {
            sessionStorage.setItem('tt_result_meta', JSON.stringify({
                gamification: gamification,
                mode: send_data.typing_mode,
                lang: send_data.ngon_ngu
            }));
        } catch (e) { /* sessionStorage unavailable */ }
        document.location.href = `${resultUrl}?r=${encodeURIComponent(result_query_param)}`;
    }

    // --- CÁC HÀM TIỆN ÍCH và HÀM TẢI BẢNG XẾP HẠNG (ĐÃ THÊM LẠI) ---

    function randomize_text(texts) {
        const words = texts.split(/[\s|]+/g).filter(Boolean);
    ...
    function set_time(timer) {
        const minutes = Math.floor(timer / 60);
        let seconds = timer % 60;
        if (seconds < 10) seconds = "0" + seconds;
        if (elements.timer) elements.timer.textContent = `${minutes}:${seconds}`;
    }
    ...
    async function _fetchAndInjectHtml(url, container) {
        if (!container) return;
        try {
            ...
            console.error(`Failed to fetch and inject HTML from ${url}:`, e);
        }
    }

    async function load_highscore_top_ranking(mode) {
        const container = document.querySelector('.ranking-table__top-ranking');
        if (!container || !window.sp) return;
        try {
            const { data: results } = await window.sp.from('typing_results').select('wpm, accuracy, user_id').order('wpm', { ascending: false }).limit(20);
            if (results) {
                const userIds = [...new Set(results.map(t => t.user_id).filter(Boolean))];
                let profilesMap = {};
                if (userIds.length > 0) {
                    const { data: profiles } = await window.sp.from('profiles').select('id, display_name').in('id', userIds);
                    if (profiles) profiles.forEach(p => { profilesMap[p.id] = p.display_name; });
                }
                let html = '<table class="table" style="width:100%;text-align:center;"><thead><tr><th>Hạng</th><th>Tên</th><th>WPM</th><th>Độ chính xác</th></tr></thead><tbody>';
                results.forEach((r, idx) => {
                    const userName = r.user_id && profilesMap[r.user_id] ? profilesMap[r.user_id] : 'Ẩn danh';
                    html += `<tr><td><span style="display:inline-block;width:24px;height:24px;line-height:24px;border-radius:50%;background:#f0f0f0;">${idx + 1}</span></td><td style="font-weight:500;">${userName}</td><td><b style="color:#527a1e">${r.wpm}</b></td><td>${r.accuracy}%</td></tr>`;
                });
                html += '</tbody></table>';
                container.innerHTML = html;
                const rt = document.querySelector('.ranking-table');
                if (rt) rt.style.visibility = 'visible';
            }
        } catch (e) { console.error(e); }
    }

    async function load_highscore_tests_taken() {
        const container = document.querySelector('.ranking-table__tests-taken');
        if (!container) return;
        container.innerHTML = '<div style="padding:40px;text-align:center;color:#666;font-size:16px;">Đang đếm số lượng... (Coming soon)</div>';
        const rt = document.querySelector('.ranking-table');
        if (rt) rt.style.visibility = 'visible';
    }

    async function load_highscore_global_challenge() {
        const container = document.querySelector('.ranking-table__global-challenge');
        if (!container || !window.sp) return;
        try {
            const { data: results } = await window.sp.from('typing_results').select('wpm, accuracy, user_id').order('created_at', { ascending: false }).limit(20);
            if (results) {
                const userIds = [...new Set(results.map(t => t.user_id).filter(Boolean))];
                let profilesMap = {};
                if (userIds.length > 0) {
                ...
                let html = '<table class="table" style="width:100%;text-align:center;"><thead><tr><th>Thứ tự</th><th>Tên</th><th>WPM</th><th>Độ chính xác</th></tr></thead><tbody>';
                results.forEach((r, idx) => {
                    const userName = r.user_id && profilesMap[r.user_id] ? profilesMap[r.user_id] : 'Ẩn danh';
                    html += `<tr><td>${idx + 1}</td><td style="font-weight:500;">${userName}</td><td><b style="color:#f57c00">${r.wpm}</b></td><td>${r.accuracy}%</td></tr>`;
                });
                html += '</tbody></table>';
                container.innerHTML = html;
                const rt = document.querySelector('.ranking-table');
    ...
    function load_random_highscore_table(mode) {
        const rand_highscore = Math.floor(Math.random() * 3) + 1;
        if (rand_highscore === 1) {
            load_highscore_top_ranking(mode);
            document.querySelector("#top-ranking-tab") ?.click();
        } else if (rand_highscore === 2) {
            load_highscore_tests_taken();
            document.querySelector("#tests-taken-tab") ?.click();
        } else {
            load_highscore_global_challenge();
            document.querySelector("#global-challenge-tab") ?.click();
        }
        const rankingTableTab = document.querySelector("#rankingTableTab");
        if (rankingTableTab) rankingTableTab.style.display = 'flex';
    }

    function get_top_user_active_all_time(element) {
        _fetchAndInjectHtml('/speedtest/get-top-test-taken?all_time=true', element);
    }

    function get_top_user_24h(element) {
        _fetchAndInjectHtml('/speedtest/get-top-test-taken', element);
    }

    function get_top_50_wpm_24h(element, mode) {
        _fetchAndInjectHtml(`/speedtest/get-top-ranking?is_50=true&mode=${mode}`, element);
    }

    function get_top_ranking_all_time(element, mode) {
        _fetchAndInjectHtml(`/speedtest/get-top-ranking?mode=${mode}&all_time=true`, element);
    }

    function get_top_ranking_24(element, mode) {
        _fetchAndInjectHtml(`/speedtest/get-top-ranking?mode=${mode}`, element);
    }
    ...
    function loadingAnimate() {
        if (elements.ajaxLoad) elements.ajaxLoad.style.display = 'block';
        if (elements.reloadBox) elements.reloadBox.style.opacity = '0.4';
    }

    function stopLoadingAnimate() {
        if (elements.ajaxLoad) elements.ajaxLoad.style.display = 'none';
        if (elements.reloadBox) {
            elements.reloadBox.style.opacity = '1';
            elements.reloadBox.style.display = 'block';
        }
    }
    ...
    function bindCoreEvents() {
        ...
        if (elements.inputfield) {
            elements.inputfield.addEventListener("compositionstart", () => {
                state.isComposing = true;
            });
            elements.inputfield.addEventListener("compositionend", () => {
    ...
    // THÊM LẠI: Hàm bind event cho bảng xếp hạng
    function bindGlobalListeners() {
        document.body.addEventListener('click', (e) => {
            const testsTakenTable = e.target.closest('.ranking-table__tests-taken');
            if (testsTakenTable) {
                if (e.target.parentElement.matches('.link-get__test-taken-24')) {
                    e.preventDefault();
                    get_top_user_24h(testsTakenTable);
                }
                if (e.target.parentElement.matches('.link-get__test-taken-all')) {
                    e.preventDefault();
                    get_top_user_active_all_time(testsTakenTable);
                }
            }
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        if (!elements.row1 || !elements.typingMode ?.getAttribute("value") || document.getElementById("competition-id")) return; // only run on typing test pages (need #row1), not result/competition pages
        const style = document.createElement('style');
        style.textContent = `#row1 { position: relative; transition: transform 150ms ease-in-out; } .wordlist { white-space: pre-line; }`;
        document.head.appendChild(style);
        bindCoreEvents();
        bindGlobalListeners(); // THÊM LẠI: Gọi hàm bind event
        restart();
    });

    // THÊM LẠI: Mở rộng Public API để các hàm có thể được gọi từ bên ngoài
    return {
        restart,
        randomize_text,
        set_time,
        ...
        load_highscore_tests_taken,
        load_highscore_global_challenge,
        load_random_highscore_table,
        get_top_user_active_all_time,
        get_top_user_24h,
        get_top_50_wpm_24h,
        get_top_ranking_all_time,
        get_top_ranking_24
    };
})();