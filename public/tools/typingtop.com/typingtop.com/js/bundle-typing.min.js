...
const TypingTest = (() => {
    ...
    function handleWordsData(data) {
        try {
            ...
            const word_string = randomize_text(data.data.word);
    ...
    async function finishTest() {
        ...
        if (!logged_user) {
            ...
            try {
                const response = await fetch("/typing/luu_kqua", {
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                if (res.success) {
                    ...
                    // res.data.accuracy may be "80,6 %" (locale-formatted) — extract numeric value
                    const rawAcc = parseFloat(String(res.data.accuracy).replace(/[^0-9.,]/g, '').replace(',', '.'));
                    accuracy = isNaN(rawAcc) ? 0 : rawAcc;
                    result_query_param = res.data.r; // token ký số do server cấp
                }
                gamification = res.gamification || null;
            } catch (e) { /* Silent fail on purpose */ }
        }

        const resultUrl = document.location.href.includes('advanced-typing-test') ? 'result-advanced-typing-test' : 'result-typing-test';
    ...
    function load_highscore_top_ranking(mode) {
        _fetchAndInjectHtml(`/speedtest/get-top-ranking?mode=${mode}`, document.querySelector('.ranking-table__top-ranking'));
    }

    function load_highscore_tests_taken() {
        _fetchAndInjectHtml('/speedtest/get-top-test-taken', document.querySelector('.ranking-table__tests-taken'));
    }

    function load_highscore_global_challenge() {
        _fetchAndInjectHtml('/speedtests/get-top-global', document.querySelector('.ranking-table__global-challenge'));
    }
